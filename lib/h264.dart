...
class ProfileLevelId {
  ...
  static final ProfileLevelId defaultProfileLevelId =
      ProfileLevelId(profile: ProfileConstrainedBaseline, level: Level3_1);
...
ProfileLevelId? parseProfileLevelId(String str) {
  ...
  ProfilePattern? pattern = ProfilePattern.patterns.firstWhere((pattern) =>
...
ProfileLevelId? parseSdpProfileLevelId([dynamic params]) {
  ...
  if (profileLevelId == null) {
    return ProfileLevelId.defaultProfileLevelId;
  } else {
...
String? generateProfileLevelIdForAnswer(
    [dynamic localSupportedParams, dynamic remoteOfferedParams]) {
  localSupportedParams ??= {};
  remoteOfferedParams ??= {};
  // If both local and remote params do not contain profile-level-id, they are
  // both using the default profile. In this case, don't return anything.
  if (localSupportedParams['profile-level-id'] == null &&
  ...
  if (localProfileLevelId == null) throw TypeError();

  if (remoteProfileLevelId == null) throw TypeError();
  ...
  var levelAsymmetryAllowed = (_isLevelAsymmetryAllowed(localSupportedParams) &&
  ...
  return profileLevelIdToString(
      ProfileLevelId(profile: localProfileLevelId.profile, level: answerLevel));
}
...
bool _isLevelAsymmetryAllowed([params]) {
  ...
  return (levelAsymmetryAllowed == 1 || levelAsymmetryAllowed == '1');
}
